...
describe("background param parsers", () => {
  it("parses terminal start params", () => {
    assert.deepEqual(parseStartTerminalParams({ sessionId: "s1", command: "npm run dev", title: "dev" }), {
      sessionId: "s1",
      command: "npm run dev",
      title: "dev",
    });
  ...
  it("rejects terminal start params without a command", () => {
    assert.throws(() => parseStartTerminalParams({ sessionId: "s1" }), /command/);
  });

  it("parses prompt start params", () => {
    assert.deepEqual(parseStartPromptParams({ sessionId: "s1", prompt: [{ type: "text", text: "hi" }] }), {
      sessionId: "s1",
...
describe("BackgroundJobManager", () => {
  it("tracks prompt output and completion", () => {
    ...
    assert.equal(readFileSync(job.outputPath!, "utf8").endsWith("hello world"), true);
  ...
  it("lists jobs by session", () => {
    const outputDir = mkdtempSync(join(tmpdir(), "nova-background-"));
    const manager = new BackgroundJobManager(outputDir);
    manager.createTerminalJob({ sessionId: "s1", command: "npm run dev", title: "dev", terminalId: "t1" });
    manager.createTerminalJob({ sessionId: "s2", command: "npm test", title: "test", terminalId: "t2" });
