...
export type BackgroundJobStatus = "running" | "completed" | "failed" | "killed" | "released";
...
const DEFAULT_BACKGROUND_JOBS_DIR = join(homedir(), ".nova-ai", "background-jobs");

export class BackgroundJobManager {
  private readonly jobs = new Map<string, BackgroundJob>();

  constructor(private readonly outputDir = DEFAULT_BACKGROUND_JOBS_DIR) {}

  createPromptJob(params: { sessionId: string; title: string; abortController: AbortController }): PromptJob {
  ...
  createTerminalJob(params: { sessionId: string; command: string; title: string; terminalId: string }): TerminalJob {
    const now = new Date().toISOString();
    const jobId = crypto.randomUUID();
  ...
  finish(jobId: string, status: Exclude<BackgroundJobStatus, "running" | "released">, details: Partial<BackgroundJobSummary> = {}): BackgroundJobSummary {
...
function writeArtifact(outputDir: string, job: BackgroundJob, output: string): void {
...
function appendArtifact(outputDir: string, job: BackgroundJob, text: string): void {
  if (!job.outputPath) return;
  mkdirSync(outputDir, { recursive: true });
  appendFileSync(job.outputPath, text, "utf8");
}

function requireJob(jobs: Map<string, BackgroundJob>, jobId: string): BackgroundJob {
...
export function parseStartPromptParams(params: unknown): StartPromptParams {
  const value = requireRecord(params);
  const sessionId = requireString(value.sessionId, "sessionId");
  if (!Array.isArray(value.prompt)) throw new Error("prompt must be an array of ACP content blocks");
  const title = optionalString(value.title, "title");
